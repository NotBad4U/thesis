...
Cette thèse répond à ce besoin en développant un cadre de reconstruction de preuves Alethe dans Lambdapi \cite{lambdapi}.
Lambdapi est un assistant de preuve fondé sur le $\lambda\Pi$-calcul modulo théories, conçu comme un langage pivot pour échanger des preuves entre systèmes \cite{thire:tel-03224039, blanqui:hal-04613926}.
Comme l’illustre la \Cref{fig:fr-interop-intro}, de nombreux assistants et autre outils peuvent exporter ou importer des preuves via Lambdapi, qui joue alors le rôle de ``langage assembleur'' pour les systèmes de preuve.
...
L’exemple du \cref{lst:fr-smtexampleinput-fol} illustre ce principe : à partir du problème d’entrée, le solveur produit une trace Alethe composée d’étapes \kw{assume} et \kw{step}, aboutissant à la clause vide \kw{(cl)}.
...
Pour analyser et reconstruire de telles traces, nous décrivons maintenant la structure générale d’une étape Alethe, résumée à l'\cref{eq:fr-step}:
...
Une étape est constituée d’un indice \colorbox{midpurple!30}{$i$} $\in \mathbb{I}$, où $\mathbb{I}$ est un ensemble infini dénombrable d’indices (par exemple \kw{a0}, \kw{t1}), et d’une clause de formules \colorbox{Emerald!30}{$l_1, \dots, l_n$} pouvant être interprétée comme une disjonction $n$-aire.
...
Les règles de preuve \colorbox{rxpurple2!30}{$\mathcal{R}$} incluent des lemmes de théorie et d'autre règle de logique comme la règle \kw{resolution} (ligne 6), qui correspond à l’hyper-résolution sur des clauses.
...
L’élaboration peut notamment: éliminer des pas redondants, expliciter des réordonnancements de clause implicites, décomposer des règles trop grossières en sous-étapes plus élémentaires, ou remplacer des simplifications complexes par des séquences compatibles.
Néanmoins, une preuve élaborée augmente le taux de reconstruction réussie au détriment d'une complexité spatiale et temporel plus forte i.e. la preuve aura plus d'étapes est sera donc plus difficile à valider.

Comme le présente l'\cref{fig:fr-bench-pipeline}, nous tirons partie du module d'élaboration des preuves Alethe par Carcara afin ensuite de les traduires en Lambdapi pour les vérifiers.
Nous illustrons ceci avec l'élaboration de la preuve du \cref{lst:fr-smtexampleinput-fol}. Carcara analyse la résolution est indique les pivots dans les arguments \colorbox{rxpink!30}{$[a_1 \dots a_r]$} de l'étape, de plus,il ordonne les premises \{\colorbox{darkpurple!30}{$p_1 \dots p_m$}\}.
...
Ainsi, comme on peut le constater dans le \cref{lst:fr-resolution-ex} ci-dessus, les étapes \kw{t3} et \kw{t4} sont maintenant détaillés avec les pivots et l'ordre des résolutions facilitatant la reconstruction dans un assistant à la preuve comme Lambdapi.
...
Lambdapi est une implémentation du $\lambda\Pi$-calcul modulo théories dénoté $\lpm$ \cite{lambdapi}, une extension du \emph{Edinburgh Logical Framework} $\lambda\Pi$ \cite{lf}, un $\lambda$-calcul simplement typé avec types dépendants.
Le système $\lpm$ ajoute la possibilité de définir des règles de réécriture d’ordre supérieur. Sa syntaxe est donnée par la grammaire suivante:
...
La relation $\hookrightarrow_{\beta\Sigma}$ est engendrée par la $\beta$-réduction et par les règles de réécriture de $\Sigma$. On note $\hookrightarrow_{\beta\Sigma}^*$ sa clôture réflexive et transitive, et $\equiv_{\beta\Sigma}$ la clôture réflexive i.e. la conversion, symétrique et transitive de $\hookrightarrow_{\beta\Sigma}$.
...
Les règles de typage sont analogues à celles de $\lambda\Pi$ \cite[\S 2]{lf}, à ceci près que la règle Conv (\cref{fig:fr-conv-rule}) identifie les types modulo $\equiv_{\beta\Sigma}$ et non seulement modulo à $\beta$-réduction.

\subsection*{Encodage de la logique SMT en Lambdapi}

Nous rassemblons ici un aperçu des types, connecteurs logiques, et des règles de réécriture qui constituent la signature $\Sigma$ utilisée dans notre encodage de la logique SMT.
...
\begin{align*}
\kw{classic}:\ & \Pi [p: \prop], \prf (p \lor \neg p) \\
...
Nous présentons ici les fonctions pour la traduction qui permettent de passer d’une trace Alethe, fondée sur la signature SMT-LIB, à des objets bien typés dans Lambdapi.
La signature SMT-LIB est décrite dans~\cite[\S 5.2.1]{smtlib}. Les détailles pour la traduction peuvent être trouvée au \cref{ch:encoding}.
...
Alethe ne prend pas en charge les sortes \kw{Array} et \kw{String}. De plus, notre reconstruction ne supporte pas encore \texttt{Bitvector} et \texttt{Real}.
...
Nous illustrons l'utilisations de ces quatres fonction pour la traduction de Alethe vers Lambdapi sur les définitions du problème d'entrée du \cref{lst:fr-smtexampleinput-fol}.
...
Les détailles peuvent être trouvée au \cref{ch:reconstruction-ul} pour la logique du premier ordre, et \cref{ch:reconstruction-la} pour l'arithmetique.
...
L'étape \kw{t1} peut-être reconstruit directement en Lambdapi en encodant le lemme génériques:
\[
...
Une hyper-résolutions est ainsi reconstruite par l'application en chaine de résolution binaire via le langage de tactique de Lambdapi. 
...
Nous présentons ici un court extrait de nos résultats; les détails de la configuration et l’ensemble des mesures sont donnés aux \cref{ch:evaluation} et \cref{app:complete-benchmarks-results}.
...
Nous reportons les résultats au format Succès--Erreur--Temps Écoulé (S--E--T). Une limite de temps de 30 secondes est donnée pour cvc5 et Carcara, et de 20s pour Lambdapi.
...
Le \cref{tab:fr-check-nb} résume les échantillons qui ont pu être élaborés, puis traduits, ensuite vérifiés par Lambdapi, et le \cref{tab:fr-temps-distribution} donne des statistiques sur les temps de vérification.
Sur le corpus \emph{Sledgehammer}, nous obtenons un taux de réussite de vérification proche de 90\,\%.
Ces résultats confirment que la vérification est généralement bien sous la seconde sur ces familles. Nous investigons les goulots d’étranglement dans la conclusion du \cref{ch:evaluation}.
...
En reconstruisant des preuves Alethe dans Lambdapi, on obtient ainsi des certificats vérifiables indépendamment, et potentiellement exportables vers d’autres assistants.
Nos campagnes expérimentales ont également permis d’identifier des anomalies ensuite corrigées, notamment dans la génération de certificats de cvc5.
...
Plusieurs perspectives s’ouvrent naturellement. Étendre la couverture à d’autres théories importantes, comme les bit-vectors, renforcerait encore l’universalité du cadre.
Enfin, approfondir l’intégration avec d’autres assistants (export vers Rocq, Lean, etc.) contribuerait à faire d’Alethe un véritable format d’échange de preuves, au service d’une vérification formelle plus transparente et plus digne de confiance.
