...
L’opérateur $\pp$ modélise la concaténation de clauses (associative à gauche via réécriture), et nous
...
Nous présentons ici les fonctions de traduction qui permettent de passer d’une trace Alethe, fondée sur la signature SMT-LIB, à des objets bien typés dans Lambdapi.
...
L'étape \kw{t1} peut-être reconstruit directement en Lambdapi en encodant le lemme génériques $\kw{equiv\_pos2}: \Pi [a: \prop] \Pi [a: \prop], \pid (\neg (a = b) \veedot \neg a \veedot b \veedot \nil)$ et similairement pour la congruence. 
Ensuite, les règles de déduction comme la \kw{resolution} utilisés aux étapes \kw{t3} et \kw{t4} sont reconstruites par un script de preuve où nous définissons un lemme générique pour la résolution binaire (\cref{th:resolution}).
...
\begin{table}[tb]
\caption{Temps de vérification Lambdapi (ms).}
\pgfplotstabletypeset[
  col sep=comma,
  font=\ttfamily\scriptsize,
  every even row/.style={
  ...
  columns/median/.style={column name={\color{RoyalBlue}Médiane}, postproc cell content/.append style={/pgfplots/table/@cell content/.add={\color{RoyalBlue}}{}}},
  columns/q3/.style={column name={Q3}},
  columns/max/.style={column name={Max}, string type, string replace={21301}{Timeout},string replace={20511}{Timeout}},
...
Nous utilisons des échantillons des familles SMT-LIB couvrant exactement les fragments pris en charge : UF, LIA, UFLIA et QF\_LIA.
...
Un échantillon representatif de chaque logique supportée est présenté par tableaux.
Nous reportons les résultats au format Succès--Erreur--Temps (S--E--T). Une limite de temps de 30 secondes est donnée pour cvc5 et Carcara, et de 20s pour Lambdapi.
La \cref{tab:fr-pass-elab} montre que, dans la grande majorité des cas, cvc5 produit un certificat de preuve et que Carcara parvient ensuite à l’élaborer, ce qui permet sa traduction et sa vérification.
La \cref{tab:fr-check-nb} résume les échantillons effectivement traduits puis vérifiés par Lambdapi, et la \cref{tab:fr-temps-distribution} donne des statistiques sur les temps de vérification.
...
Elle propose un cadre de reconstruction qui traduit des traces Alethe en preuves vérifiables par le noyau de Lambdapi,
via un encodage modulaire de la logique SMT (raisonnement du premier ordre, égalité, et arithmétique linéaire) et des techniques de reconstruction par réflexion.
L’évaluation sur des familles SMT-LIB représentatives montre que, dès aujourd’hui, la chaîne de bout en bout peut vérifier efficacement un grand nombre de preuves, la plupart en quelques centaines de millisecondes.
Au-delà de ces benchmarks, nous avons également mis ces travaux à contribution pour valider des preuves manipulées dans Lambdapi lorsqu’il s’appuie sur des solveurs SMT comme arrière-plan ; cet usage, non détaillé ici, est discuté dans le chapitre d’évaluation.
En reconstruisant des preuves Alethe dans Lambdapi, on obtient des certificats vérifiables indépendamment, et potentiellement exportables vers d’autres assistants.
Enfin, nos campagnes expérimentales ont permis d’identifier des anomalies ensuite corrigées, tant dans la génération de certificats de cvc5 que dans certains encodages SMT de la théorie des ensembles, qui pouvaient conduire à des preuves invalides.
...
Enfin, approfondir l’intégration avec d’autres assistants (export vers Rocq, Isabelle/HOL, Lean) contribuerait à faire d’Alethe un véritable format d’échange de preuves, au service d’une vérification formelle plus transparente et plus digne de confiance.
