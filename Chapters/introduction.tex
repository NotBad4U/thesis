...
The \ac{SMT} solvers~\cite{cvc5,verit} rank among the most advanced automated reasoning tools currently available and have become indispensable in a wide variety of formal reasoning tasks.
These include program verification and interactive theorem proving.
...
A first alternative is to develop a certified solver in a proof assistant environment such as the IsaSAT solver \cite{fleury:tel-02963301,fleury:hal-01904647} that is a fully verified \ac{SAT} solver developed in Isabelle/HOL \cite{isabelle-hol-ref}.
...
As the example \cref{lst:alethe-ex-intro} shows, the format is close to SMT-LIB and facilitates proof production.
However, it lacks a standalone checker, which harms its usability and hinders its adoption. 
Moreover, the coarse-grained steps can be too expensive to check and lead to verification failures.

\lstinputlisting[language=SMT,label={lst:smt-ex-intro},caption={A Simple SMT Problem}]{Assets/example_uf.smt2}

\lstinputlisting[language=SMT,label={lst:alethe-ex-intro},caption={Alethe Proof of \cref{lst:smt-ex-intro} Found by cvc5.}]{Assets/example_uf.proof}


\begin{figure}[b]
    \centering
    \begin{tikzpicture}
      \path (0,0) node (lp) {\textcolor{purple}{Lambdapi}}
            (-4,1) node (coq) {Rocq}
            (-3,2) node (lean) {Lean}
            ...
            ;
      \draw[->,purple, thick] (smt) -- (lp) node[midway,sloped,above] {};
    ...
    \end{tikzpicture}
    \caption[Lambdapi as proof interchange]{Lambdapi, an assembly language for proof systems.}
