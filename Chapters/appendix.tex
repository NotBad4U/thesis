...
\begin{lstlisting}[language=trs, caption={[TRS rules for Z and P] Rewriting rule of $\bb{Z}$ and $\bb{P}$ in the TRS format}]
(VAR
  a: Z
  b: Z
  x : P
...
\section[Translation example with more Alethe features]{Translation example involving more Alethe features}
...
\lstinputlisting[language=SMT,caption={[Alethe proof of the input example] Alethe Proof of \cref{lst:input-ex-app} Found by cvc5},label={lst:proof-ex-app}]{Assets/app_ex.alethe}
...
{%
\tiny
\caption[cvc5 time measurements]{Time measurements for cvc5 in seconds. }
...
{%
\tiny
\caption[Carcara elaboration times]{Time measurements for Carcara elaboration.} 
...
\end{axis}
\end{tikzpicture}
\caption[Translation times for small problems]{Lambdapi translation times for small problems.}
...
\end{axis}
\end{tikzpicture}
\caption[Translation times for large problems]{Lambdapi translation times for large proofs.}
