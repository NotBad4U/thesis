...
Since Alethe extend the SMT format with the \smtinline{choice} operator (\cite[\S 2.1]{alethespec}), we add it to the prelude. 
...
SMT-LIB provides the operator \smtinline{(distinct A A Bool)} that denotes the function that
returns true if its two arguments are not identical as defined in \cref{tab:distinct-spec}, and \smtinline{(ite Bool A A A)} denotes the function that
