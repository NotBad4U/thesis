...
\begin{table}[tb]
\caption[Specification of \texttt{distinct\_elim}]{Specification of the \tt{distinct\_elim} rule in Alethe.}
\begin{tabular}{@{}ll@{}}
...
\textit{i.} $\Gamma \triangleright \ (\mathsf{distinct}\ \varphi\ \psi) \approx \lnot(\varphi \approx \psi)$ & \tt{(R)[]} \\[0.3em]
\textit{i.} $\Gamma \triangleright \ (\mathsf{distinct}\ \varphi_1\ \varphi_2\ \varphi_3\ \ldots) \approx \bot$ & \tt{(R)[]} \\[0.8em]

\multicolumn{2}{@{}l@{}}{For all other sorts the rule is} \\[0.5em]
\textit{i.} $\Gamma \triangleright \ (\mathsf{distinct}\ t_1 \ \ldots \ t_n) \approx \bigwedge_{i=1}^{n} \ \bigwedge_{j=i+1}^{n} \ t_i \neq t_j$ & \texttt{(R)[]}
\end{tabular}
...
Alethe distinguishes between clauses that appear in steps, such as \colorbox{Emerald!30}{(cl~$l_1 \dots l_n$)} in \cref{eq:step}, and ordinary disjunction \cite[\S 4]{alethespec}.
