...
\begin{example}
We can define $\frac{0}{1}$ as $\kw{Rat}~0~1~\top_i$.
\end{example}

Two integers a and b are coprime (with $|\_|$ the absolute value function), if the only positive integer that is a divisor of both of them is $1$.
The function $\kw{Zgcd}$ returns the Greatest Common Divisor $(\mathop{GCD})$ between two terms of type $\Z$.
...
We first define the product type in \cref{def-product}, with the constant $\times: \set \ra \set \ra \set$ written infix.

\begin{definition}[Product type]\label{def-product}
\begin{align*}
&\textbf{Non dependent product type:} \\
&\quad \times : \text{Set} \to \text{Set} \to \text{Set} \\
...
The constructor function $\kw{fracq}$ ,in \cref{def:fracq-def}, normalizes any integer pair to this reduced form, so if two inputs represent the same rational number
$\kw{fracq}$ returns exactly the same $\Q$ value (with identical numerator/denominator and proof).
...
& \kw{fracq}~n~d \re \kw{Rat}~(\kw{fracq\_subdef }~(n, d)) \\
\end{align*}
...
& \qquad \prf (\kw{istrue} (\kw{isLt} (0 \doteq p_2) \mathbin{and} \kw{coprime}\,p_1\,p_2)) \\
&\text{and}\\
...
& \quad \mathop{else}~\mathop{if}~\mathop{then}~(y \neq 0) \\
& \qquad \mathop{else}~\mathop{let}~g \is \kw{Zgcd}(x,y)~ \mathbin{in}  \\
& \qquad (-1^{(IsLt0(x) + IsLt0(y)))} * \mathop{div}~|x|~g‚ \mathop{div}~|y|~g) \\
\end{align*}
Where
\begin{itemize}
\item $\kw{fracq\_subproof}$ :  A lemma proving that the result of:\\
  $\kw{fracq\_subdef}$ satisfies the required properties,
...
We can then define the regular arithmetic operator over $\Q$ by using the $\kw{fracq}$ function.
For example, we define the addition as follows:

\begin{align*}
&q+ : \Q \ra Q \ra \Q (\text{written infix})\\ 
&(\kw{Rat}~n_1~d_1~\_) q+ (\kw{Rat}~n_2~d_2~\_) \re\\
&\qquad \kw{fracq}~((n_1 * d_2) + (n_2 * d_1)) (d_1 * d_2) \\
\end{align*}
