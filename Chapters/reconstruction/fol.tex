...
In our encoding of the \kw{resolution} rule, we will represent clauses as disjunctions of literals constructed from $\List{o}$, using the function $\kw{disj}$, rather than the abstract type $\kw{Clause}$.
...
\begin{align*}
&\kw{disj} : \List{o} \ra \el~o \\[0.3em]
&\kw{disj}~(l_1~\cons~l) \re (l_1 \lor \kw{disj}~l) \\[0.3em]
&\kw{disj}~\nil \re \bot
\end{align*}
\end{definition}

To avoid explicit conversions, we rely on Lambdapi's coercion mechanism (detailed in \cref{ssec:coercion}) to implicitly translate between $\kw{Clause}$ and list, allowing the two representations to be used interchangeably in proofs.
...
Let $ps$ and $qs$ be two $\List{o}$ and two index $i,j \in \mathbb{N}$. If
\[
...
\begin{itemize}
  ...
  \item[] Case $qs=t \cons \ell$ with the induction hypothesis saying that for every clause $xs$ and every pair of indices $n,m$, if $\prf(\kw{disj}~xs)$ and $xs$ is equal to the subclause of $\ell$ from position $n$ up to $m$, then $\prf(\kw{disj}~\ell)$.
    ...
    \begin{itemize}
      \item For $j=0$, we have $\mathrm{take}~0~(t \cons \ell)=\Box$, hence the subclause $ps$ of $qs$ is empty i.e. $ps = \Box$.
      ...
      \item When $j > 0$, we then analyze $i$.
        \begin{itemize}
          \item If $i=0$, then
            \[
              \mathrm{subclause}(t \cons \ell,0,j{+}1)= t \cons \mathrm{subclause}(\ell,0,j).
            \]
            and the equality hypothesis yields
            \[
              \prf \bigl( \kw{disj}~ps = t \lor \mathrm{take}~j~\ell \bigr).
            \]
            ...
            Otherwise, $\prf(\kw{disj}~(\mathrm{take}~j~\ell))$ holds; by the induction hypothesis we lift provability to $\prf(\kw{disj}~\ell)$, and then conclude $\prf(\kw{disj}~t \cons \ell)$ by right introduction.
  ...
  \begin{itemize}
    ...
    \item If $j = j' +1$, we need to show that
      ...
      So it is enough to prove that:
       \[
          \ell[j'+1] \lor t \lor \kw{disj}~(\kw{remove}~\ell~j') \bigr) = t \lor \kw{disj}~\ell
      \]
      By the induction hypothesis applied to $\ell$ at index $j'$, we know that:
      \[
        \prf\bigl(\,\ell[j'] \lor (\kw{disj}~(\kw{remove}~\ell~j')) = \kw{disj}~\ell\,\bigr).
      \]
      Combining this with the identities above, we obtain
      \begin{flalign*}
        l[j'] \lor t \lor \kw{disj}~(\kw{remove}~\ell~j') &= \kw{disj}~(t \cons \ell) && \\
        &= t \lor \kw{disj}~\ell  && \\
