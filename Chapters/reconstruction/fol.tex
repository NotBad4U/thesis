...
We first prove the following lemmas necessary to prove our resolution rule.
  ...
  \item[] Case $qs=t \veedot \ell$ with the induction hypothesis saying that for every clause $xs$ and every pair of indices $n,m$, if $\prfcl(xs)$ and $xs$ is equal to the subclause of $\ell$ from position $n$ up to $m$, then $\prfcl(\ell)$.
    ...
    \begin{itemize}
      ...
      \item When $j > 0$ we then analyze $i$.
        \begin{itemize}
          \item If $i=0$, then $\mathrm{subclause}(t \veedot \ell,0,j{+}1)= t \veedot \mathrm{subclause}(\ell,0,j)$.
          ...
          \item If $i > 0$, we peel one element from both $\mathrm{drop}$ and $qs$.
            The equality premise specializes to $ps=\mathrm{subclause}(\ell,i,j)$, and the desired claim follows from the induction hypothesis on $\ell$.
          \end{itemize}
    \end{itemize}
...
Let $qs$ be a clause and $j < \mathrm{size}(qs)$. Then
\[
...
\begin{itemize}
  ...
  \[
    \prfcl((t \veedot \ell)[j] \veedot \kw{remove}(t \veedot \ell,j) = t \veedot \ell).
  \]
  We proceed by a case analysis on $i$.
  \begin{itemize}
    \item with $j=0$. Then $\kw{remove}(qs,0)=\ell$ and $qs[0] = t$.
    \item When $j>0$,
      then $\kw{remove}~qs~j = t \veedot (\kw{remove}~\ell~j-1)$ and 
      $\kw{nth}(\bot,qs,j)=\kw{nth}(\bot,\ell,j-1)$.  
      By the induction hypothesis on $\ell$ at position $j-1$, we have
      \[
        \prfcl(\kw{remove}(\ell,j-1)) \;\veedot\; \kw{nth}(\bot,\ell,j-1) \;=\; \prfcl(\ell).
      \]
      Adding $t$ on the left and using associativity of $\veedot$, we conclude that
      \[
        \prfcl(\kw{remove}(qs,j)) \;\veedot\; \kw{nth}(\bot,qs,j) \;=\; \prfcl(qs).
      \]
  \end{itemize}
\end{itemize}
...
% From Lemma~\ref{lem:orN} applied to $(qs,j)$ we know that
% \[
%   \prf\bigl(\mathrm{disj}(qs)\bigr) \;\Rightarrow\;
%   \prf\bigl(\mathrm{disj}(\kw{remove(qs,j)) \lor Q\bigr).
% \]
% Since $\prf(\mathrm{disj}(qs))$ is given, this yields
% $\prf(\mathrm{disj}(\kw{remove(qs,j)) \lor Q)$.
% But under the assumption $\lnot Q$, the disjunct $Q$ is false, so the disjunction reduces to
% $\prf(\mathrm{disj}(\kw{remove(qs,j)))$ (by Lemma~\ref{lem:or_identity_r}).
% Now we combine this with $\kw{remove(ps,i)$ using the preservation lemma
% (Lemma~\ref{lem:orN_append_right}), which states that appending a list with a
% provable disjunction preserves provability.
% Hence we obtain
% \[
% \prf\bigl(\mathrm{disj}(\kw{remove(ps,i){++}\kw{remove(qs,j))\bigr).
% \]
% \(\prf\bigl(\mathrm{disj}(\kw{remove}(ps,i){++}\kw{remove}(qs,j))\bigr)\).

\item[] Case 2: \(\prf(\lnot ps[i])\).
% Then again by the relation $P = \lnot Q$ we deduce $\prf(Q)$.
% From Lemma~\ref{lem:orN} applied to $(ps,i)$ we know that
% \[
% \prf\bigl(\mathrm{disj}(ps)\bigr) \;\Rightarrow\;
% \prf\bigl(\mathrm{disj}(\kw{remove(ps,i)) \lor P\bigr).
% \]
% Since $\prf(\mathrm{disj}(ps))$ is given, this yields the disjunction
% $\prf(\mathrm{disj}(\kw{remove(ps,i)) \lor P)$.
% Under the assumption $\lnot P$, the right disjunct $P$ is false, so the disjunction reduces to
% $\prf(\mathrm{disj}(\kw{remove(ps,i)))$.
% Now by Lemma~\ref{lem:orN_append_left}, which allows us to append a provable
% disjunction on the left, we conclude
% \[
% \prf\bigl(\mathrm{disj}(\kw{remove(ps,i){++}\kw{remove(qs,j))\bigr).
% \]
\end{itemize}

% Since in both cases the conclusion holds, we have shown that whenever
% $ps$ and $qs$ are provable and contain complementary literals at positions $i$
% and $j$, the resolvent clause obtained by removing these literals and joining
% the remainders is also provable.
% By the isomorphism $\bb{L}o \cong \Clause$, this is precisely the usual
% resolution rule at the level of clauses.
\end{proof}
\end{theorem}
