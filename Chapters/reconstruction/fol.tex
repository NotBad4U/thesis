...
In our reconstruction we assume an assignment $\sigma$ computed automatically from the literals of the clause $l_{k_1},\, \dots,\, l_{k_m}$.
This assignment not only interprets propositional variables as truth values but also  induces an order on terms through their identifiers.  
We exploit this order to define a canonical representation of clauses: we turn a clause into a list of literal identifiers, stably sort this list using mergesort,  and then delete adjacent duplicates.  
...
We first encode a canonical algebra of literals:

\begin{align*}
&\cal{C} : \type & \kw{cl}: \set  \\
&| p: \N \ra \cal{C} & \el~\kw{cl} \re \cal{C}
\end{align*}

A clause is then represented as a list of elements of $\cal{C}$.

\begin{definition}[split]
The function \kw{split} divides a list into two sublists by alternating elements:
\begin{align*}
&\kw{split}: \bb{L}~\kw{cl} \ra \el (\kw{list}~\kw{cl} \times \kw{list}~\kw{cl})\\
...
&\kw{remove}~(p\,i :: p\,j :: tl) \re
\begin{cases}
  \kw{remove}~(p\,j :: tl) & \text{if } i = j, \\
  p\,i :: \kw{remove}(p\,j :: tl) & \text{if } i \neq j
\end{cases}
\end{align*}
\end{definition}

\begin{theorem}[Mergesort correctness]\label{thm:mergesort-correct}
For all \(g\) and context \(\sigma\),
\[
