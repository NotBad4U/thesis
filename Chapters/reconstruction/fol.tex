...
\begin{figure}[t]
  ...
  $j : \begin{array}[t]{@{}l@{}}
       ...
       \end{array}$
  \end{tabular}
  \caption[Representative Alethe translations]{Translations for three representative Alethe commands.}
  \label{fig:fun-c}
\end{figure}
...
Let the two lists of propositons $ps,qs: \List{o}$, if $\prf(\kw{disj}~qs)$ then also $\prf(\kw{disj}~(ps \concat qs))$.
\begin{proof}
...
\end{lstlisting}
The function $\kw{headcl}$ \index{\kw{headcl}} at line 11 has type $\prfcl (a \veedot \nil) \ra \prf\,a$, and $\top_i: \prf (\top)$ denote the introduction for $\top$.
\end{example}
...
\begin{figure}
  ...
  \end{tabular}
  \caption[Translation of \kw{not\_simplify}]{Translating the command \kw{not\_simplify}.}
...
\hline
\end{tabular}
\caption[Translation of subproof and bind]{Translating \kw{subproof} and \kw{bind} commands.}
...
\section{The \texttt{evaluate} cvc5 rule}\label{evaluation-recon}
...
\begin{lstlisting}[language=SMT,caption={[Proof trace using cvc5 evaluate]An example of proof trace using the cvc5 $\kw{evaluate}$ rule.},label={lst:eval-step}]
...
We define a reification function $\reify{\_} \index{$\reify{\_}$} : \mathbb{L}~o \rightarrow \prop \rightarrow \mathcal{P}$:
...
and an interpretation function $\deno{\_} \index{$\deno{\_}$} : \mathbb{L}~o \rightarrow \mathcal{P} \rightarrow \prop$:
...
&\kw{provable}~(\kw{pOther}~i)~\hyps \re (\kw{knows}~(\kw{pOther}~i)~\hyps) \\
&\hspace{11em} ~\kw{or}~ (\kw{knows}~\kw{pFalse}~\hyps)
\end{align*}
...
While this procedure is sound, it is incomplete: it may not always find proofs for all provable statements.
Empirical results, however, show that it is sufficient to handle problems like those in \cref{lst:eval-step}.
That said, the current implementation fails to prove goals involving quantifiers, such as:
$(\forall x, \forall y, x \land p~y) = (\forall x, x \land (\forall y, p~y))$.
For such cases, we still rely on the Why3 prover until the procedure is extended to support quantifiers, reducing the need for external ATP.
...
\begin{align*}
&\cal{C} \index{$\cal{C}$} : \type & \kw{cl} \index{$\kw{cl}$}: \set  \\
...
\end{tikzcd}\)
\[
  \text{with } [\_] = \kw{remove\_duplicates} \circ \kw{sort}
\]
\end{center}
...
&\Downarrow\,\sigma~(p\,i :: \ell) \re \sigma[i] \veedot \Downarrow\,\sigma\,\ell.
\end{align*}
\end{definition}
...
\begin{example}[Reification]
Given $\sigma \is x \cons y \cons \Box$, the clause:
\[
