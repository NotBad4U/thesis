...
Given an arbitrary $a : \kw{Clause}$ and predicates $p, q : \el{}\,a \ra \prop$, if $\Pi x : \el{}\,a,~ \pic (p~x = q~x)$ then $\pic ((\forall x, p~x) = (\forall y, q~y))$.
\end{lemma}

\smallskip

\begin{lemma}[bind$\exists$]\label{lem:bind-exists}
  Given an arbitrary $a : \kw{Clause}$ and predicates $p, q : \el{}\,a \ra \prop$, if $\Pi x : \el{}\,a,~ \pic (p~x = q~x)$ then $\pic ((\exists x, p~x) = (\exists y, q~y))$.
\end{lemma}
...
we obtain that $\pic (\kw{provable}~\reify{\cal{G}}_\sigma = \texttt{true})$ implies the desired assertion $\pic ((p \land \top) \lor \bot \Leftrightarrow p)$.
\end{example}
