...
\begin{figure}
  ...
  $(1) \neg \neg \varphi \Rightarrow \varphi$ &  \lpinline{eval not_simplify} \\
...
\subsection{The evaluate Alethe rule}
...
The constructors \texttt{pNeg} and \texttt{pOther} encode uninterpreted expressions, including variables.
The index $\mathbb{N}$ will used as an index into a context map $\sigma$ holding the corresponding expressions of type $\prop$.
Our automation will not look into the context mapping $\sigma$, but will be able to determine if two \texttt{pOther} (and \texttt{pNeg}) terms reference the same location and are therefore equal.
...
&\reify{\neg x}_\sigma \re  \kw{pNeg}~ (\kw{Index}(\sigma, x)) && \\
...
We define what it means for all members of a context $\hyps$ of type ($\kw{list}~\cal{P}$) to represent true propositions,
and we prove some lemmas about this notion. The predicate $\kw{knows}$ checks whether the proposition $p$ exists  in the context.
The \kw{all} function combines all the $\PP$'s in a list into a $\prop$ representing their conjunction.
We now define a decidable equality between natural numbers that we will use to compare index of \emph{Atomic} constructor.

\smallskip

\begin{definition}[Decidable equality $=_\N \colon \N \ra \N \ra \mathbb{B}$]
We define the decidable equality between two natural numbers as follows:
\begin{flalign*}
&0 =_\N 0 \re \true & &S~x =_\N S~y \re x =_\N y &&\\
&0 =_\N S~x \re \false & &S~x =_\N 0 \re \false &&
\end{flalign*}
\end{definition}
...
The function \kw{all} transform a list of $\cal{P}$ into conjonctions to represent a context.
It use a fold right function from the Lambdapi standard library\footnote{https://github.com/Deducteam/lambdapi-stdlib}.
\begin{flalign*}
\kw{all}: \kw{list}~\cal{P} \ra \cal{P} \coloneqq \kw{foldr}~(\kw{pAnd})~\kw{pTrue} &&
\end{flalign*}
The function \kw{knows} searches for an element decidably equal to $p$. It also use the function \kw{existsb} from the standard library.
\begin{flalign*}
\kw{knows}~(p: \cal{P})~(\Gamma: \kw{list}~\cal{P}) \coloneqq \kw{existsb}~(=_\cal{P})~\Gamma &&\\
\end{flalign*}

The function \kw{learn} extends the list of known facts $\Gamma$ to include one more fact. We use the standard notation $(::)$ for append an element to a list.
...
%
\begin{flalign*}
&\kw{learn}~(p: \PP)~(\hypst): \kw{list}~\PP &&\\
...
Given a mapping $\mapty$ and a context $\hypst$ then for any $\kw{p}: \PP$, if $\prf \deno{\kw{all}~\hyps}$ and $\prf \deno{p}$, then  $\prf \deno{\kw{all} (\kw{learn}~\kw{p}~\hyps)}$.
\end{lemma}
...
If our function \kw{knows} returns \true{}, then the proposition is implied by the meaning of the context:
...
%
\begin{flalign*}
&\kw{provable}~(goal: \PP)~(\hypst): \mathbb{B} &&\\
...
For any $\kw{goal}: \PP$ and context $\hypst$, if $\prf (\kw{provable}~\kw{goal}~\hyps = \true)$
