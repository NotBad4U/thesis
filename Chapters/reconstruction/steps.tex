...
\subsection{General scheme of the reflective reconstruction}
...
\subsubsection{Associative Commutative Inner Normalization}
...
\lstinputlisting[language=Lambdapi,mathescape=true,label={lst:translation-example},caption={Lambdapi proof of List. \ref{lst:smtexampleproof}}]{Assets/short_lia.lp}
...
\subsection{Reflection with outer normalization}
\info{ac}{this work is still in development compared to inner.}

We now describe the second strategy that encodes the normalization function in Lambdapi and profits of a fully internal correctness proof within Lambdapi but slower in performance.
Similarly to the first strategy, an inductive type $\bb{G}$ (\cref{outer-g-def}) defines a syntactic representation of arithmetic expressions over $\Z$.

\subsubsection{Reflecting integers}
...
\begin{align*}
& \bb{G}: \type \\
&|~\kw{add}: \bb{G} \ra \bb{G} \ra \bb{G} \\
&|~\tt{var}: \N \ra \Z \ra \bb{G} \\
&|~\tt{cst}: \Z \ra \bb{G} \\
&\tt{grp}: \set \\
&\el~\tt{grp} \re \bb{G}
\end{align*}
...
The \tt{reify} function performs syntactic reification of arithmetic expressions from $\bb{Z}$ into the abstract type $\bb{G}$, producing a pair consisting of a reified term of type $\bb{G}$, and a list of variable values called the \emph{context map}
...
The function \den{\_} defines the semantic interpretation of reified arithmetic expressions of type $\bb{G}$.
Given a pair $(g, l)$ consisting of a term $g : \bb{G}$ and a context map $l$ of integer values corresponding to uninterpreted expression (e.g. variable), the function computes the resulting value in $\mathbb{Z}$.
Variables are interpreted via lookup in the context map $l$, defaulting to zero if the index is out of bounds. The defining equations of the denotation function $\den{\_}$ are as follows:

\begin{align*}
&\den: (\bb{G} \times \bb{L}~\kw{int}) \ra \Z\\
&\den{(\Cst(k), l)} \re k \\
&\den{(\Var{i}{k}, l)} \re \text{nth}(l, i) \cdot k \\
&\den{(\Add(x, y), l)} \re \den{(x, l)} + \den{(y, l)}
\end{align*}


\begin{example}
Consider the arithmetic expression $x + y + x + 1$, where $x$ and $y$ are $\Z$ variables. 
When reifying this expression, we start with an empty list of previously seen values and apply the $\reify{}$ function recursively.
...
\subsubsection{Outer normalisation procedure}
...
\begin{itemize}
  ...
  \item $\kw{sort}: \bb{L}~\kw{grp} \ra \bb{L}~\kw{grp}$, sort aliens variables by insertion sort and combines similar terms (e.g. $\Add{\Var{0}{1}}{\Var{0}{1}}$).
...
It recursively traverses the structure and flattens it into a list of constants and variable terms, discarding the additive structure. This is used as a preprocessing step for normalization. The defining equations are:
...
This operation reassembles a flat list into a left nested addition tree. It is defined recursively as follows:
...
\subsubsection{Normalization Correctness}
...
In the following we will not mention $\prf$ to lighter the notation.

The \cref{lem:no-add-inv} asserts that the list of atomic terms produced by the function \kw{aliens} contains no nested additions. This structural invariant is crucial for the correctness of subsequent normalization steps such as sorting and zero elimination, which assume a flat list of atomic expressions.
...
If a list $l$ contains no additions, then removing all neutral elements with \kw{remove0} does not affect its semantic value:
\[
...
If the list $g :: l$ contains no additions, then inserting $g$ into $l$ preserves denotation up to addition:
\[
...
Specifically, the function \kw{norm}, which transforms an expression into a canonical form by flattening, sorting, and removing neutral elements, does not change its denotation.
