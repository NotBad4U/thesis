...
\]
Hence to prove that $P \lor Q$ and $Q \lor P$, are logically equivalent, it suffices to represent them as abstract expressions $m_1$ and $m_2$ of type $\mathsf{Monoid}$, apply the function $\mathop{norm}$ to normalize both, and check that $\mathop{norm}(m_1) = \mathop{norm}(m_2)$.
...
Proof by computational reflection is a technique introe can be substituted with one where the pivot is placed at the head position, allowing for the subsequent application of \cref{lemma:resolution}.
...
%     % \arrow["{{\reify{\_}}}"', from=3-2, to=1-2]duced in \cite{reflected_origin} that benefits from the internal reduction system of the proof assistant in order to reduce the size of the proof term computed and consequently speed up its checking. In Lambdapi, we can take advantage of the fact that rewriting rules are part of the internal reduction system ($\equiv_{\beta\cal{R}}$), which makes proof by reflection convenient to set up and implement.
...
If two clauses are determined to be equal, the current claus
...
%  $\forall (b : \cal{C}), \tt{istrue} (\reify{xs} =_{\mathcal{C}} \reify{b}) \text{ implies that } \cal{F}\, xs = \cal{F}\, b$, and
...
% %By using the induction on equality $ind\_{eq}$ with the predicate $\forall c, \tt{istrue}(is\blacksquare~c)$,
...
%   \item Lastly, suppose $c_1 = x \veedot xs$ and $c_2 = y \veedot ys$. By induction hypothesis we have $\forall (b : \tt{Clause}), xs = b \text{ implies that } \tt{istrue} (\reify{xs} =_{\mathcal{C}} \reify{b})$,
