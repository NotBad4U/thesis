...
In this section, we address the challenge of reconstructing the rule $\kw{la\_generic}$, as detailed in \cref{ssec:challenge-recon} when it is used in \textbf{LIA} logic.
This rule corresponds to a proof step in which a set of linear inequalities is shown to be contradictory.
The Alethe specification guarantees that the contradiction has to reduce to a normal form: $0 \bowtie k$ where $k$ is constant with $k > 0$, and $\bowtie \in \{ \geq, > \}$.
...
These identities are typically derived through elementary combinations of canonical equalities. Other rules are just encoded as regular lemmas
...
The procedure operates on a group structure $\bb{G}$ that will differ between the two strategies. It includes a base type for its elements and constructors to reflect $\Z$, together with the usual axioms defining a
...
Working by reflection means that we build a semi decision procedure like illustrated in \cref{fig:la-generic-comm-diagram}, which will take $t1$ and $t2$ of type $\bb{Z}$ as arguments and compare them (i.e. returns a term of type $\kw{Comp}$ ) modulo associative-commutative rewriting in the group structure.

Once we have built $g_1, g_2: \bb{G}$,  by reifying $t_1$ and $t_2$ using the function $\reify{}$,
the idea is to check the equality of the normal forms of $g_1$ and $g_2$ denoted by the function $[\_]$ and to prove
that this implies the same comparison of $t_1$ and $t_2$ in $\Z$ through a denotation function $\den{()}$. For this purpose, we should ensure the correctness of the following diagram:
...
\(\begin{tikzcd}[ampersand replacement=\&,column sep=normal]
	{g_1 \bowtie_\bb{G} g_2} \& \bb{G} \&\& \bb{G} \& {[g_1] \bowtie_\bb{G} [g_2]} \\
	\\
...
\end{center}
with $\bowtie \in \{ \geq, > \}$.
\caption[Normalization by reflection]{Reflexive normalization of linear polynomials.}
\label{fig:la-generic-comm-diagram}
\end{figure}

\section{Reflection with inner normalization}
\label{sec:inner-normalization}
...
\begin{align*}
& \bb{G}\index{$\bb{G}$}: \type \\
&|~\add{}{} \index{$\add{}{}$}: \bb{G} \ra \bb{G} \ra \bb{G} \\
&|~\tt{var} \index{$\tt{var}$}: \Z \ra \Z \ra \bb{G} \\
&|~\tt{mul} \index{$\tt{mul}$}: \Z \ra \bb{G} \ra \bb{G} \\
&|~\tt{opp} \index{$\tt{opp}$}: \bb{G} \ra \bb{G} \\
&|~\tt{cst} \index{$\tt{cst}$}: \Z \ra \bb{G} \\
&\tt{grp} \index{$\tt{grp}$}: \set \\
&\el~\tt{grp} \re \bb{G}
\end{align*}
...
Reification ($\reify{}$) and denotation ($\den{}$) functions for $\bb{G} \rightleftarrows \bb{Z}$ are defined as follows:
\begin{align*}
& \reify{} \index{$\reify{}$}: \Z \ra \bb{G} & & \den{} \index{$\den{}$}: \bb{G} \ra \Z \\
& \reify{\ZO} \re \cst{\ZO} & & \den{\cst{c}} \re c \\
& \mathop{\reify{\ZPos}} c \re \cst{(\ZPos c)} & & \den{\opp{x}} \mathrel{\re}  \mathop{\sim (\den{x})} \\
& \mathop{\reify{\ZNeg}} c \re \cst{(\ZNeg c)} & & \den{\mul{c}{x}} \re  c * (\den{x}) \\
& \reify{(x + y)} \re \add{(\reify{x})}{(\reify{y})} & & \den{\add{x}{y}} \re \den{x} \,+\, \den{y} \\
& \reify{(\sim x)} \re \opp{(\reify{x})} & & \den{\var{c}{x}} \re  c * x \\
...
\subsection[AC inner normalization]{Associative Commutative Inner Normalization}
...
&\add{\cst{c_1}}{\cst{c_2}} \re \kw{cst}(c_1 + c_2) \\
&\add{\cst{c_1}}{(\add{\cst{c_2}}{y})} \re \add{\kw{cst}(c_1 + c_2)}{y} \\
&\add{\cst{0}}{x} \re x \\
&\add{x}{\cst{0}} \re x \\
...
The relation $\ACcanon$ \index{$\ACcanon$} is defined as $\re_\Sigma\,\twoheadrightarrow^{AC}$, where $\Sigma$ contains the rewrite rules of \cref{def:arith-ops,def:grp-arith-ops}.
\end{definition}
...
\end{lemma}
\begin{proof}
AProVE \cite{aprove} proves the termination of $\rwModAC$ \index{$\mathop{\rwModAC}$}.
\end{proof}
...
{
  \var{(c_1 + c_2)}{x} \oplus (\var{c_3}{x} \oplus y) \qquad
}
{
  \qquad \var{c_1}{x} \oplus (\var{(c_2 + c_3)}{x} \oplus y)
}{3}{3}
\end{center}
\medskip
...
and normalizing them using $\twoheadrightarrow^{AC}$. Following the reduction rules specified in \cref{def:grp-arith-ops}, we can then compare their corresponding $\Z$-terms by applying the denotation function $\den{}$.
To validate this procedure, it is necessary to establish the correctness of the \cref{fig:la-generic-comm-diagram}, formally expressed by \cref{thm:normalization}.
...
The \lstinline[language=Lambdapi,basicstyle=\ttfamily\footnotesize\upshape]|rewrite Zinv_lt_eq| step at line 3 performs the normalization from Step 1 of the \cref{alg:la-generic-description}, rewriting $\neg (3 < x)$ as $\neg (\sim 3 > \sim x)$.
...
To express all inequalities uniformly, the remaining equalities are rewritten as $\geq$ using \tt{Z\_eq\_implies\_ge} (line 12).
...
This approach benefits from an entirely correctness proof at the object theory level, but it typically results in slower performance compared to the first strategy.
...
This notation will mainly be used when inspecting an element of a context map during reification and denotation.
\end{notation}
...
\begin{align*}
& \bb{G} \index{$\bb{G}$}: \type \\
&|~\kw{var} \index{$\kw{var}$}: \N \ra \Z \ra \bb{G} \\
&|~\kw{cst} \index{$\kw{cst}$}: \Z \ra \bb{G} \\[1em]
&\kw{grp}: \set \\
&\el~\kw{grp} \re \bb{G}
\end{align*}
\end{definition}
...
\begin{align*}
&\kw{mul}\index{$\kw{mul}$} : \Z \ra \List{grp} \ra \List{grp} \\
&\kw{mul}~\_~\Box \re \Box \\
...
\[
  \Uparrow \index{$\reify{\_}$} : \List{int} \ra \Z \ra \el (\tlist{grp} \times \tlist{int})
\]
...
&\reify{\ell}{(-x)} \re \mathrel{let} (g, \ell') = \reify{\ell}{x} \mathrel{in} (\kw{mul}~(-1)~g), \ell') \\
...
&\reify{\ell}{x} \re
  \begin{cases}
    ...
    (\Var{(\kw{size}(\ell))}{1} \concat \Box, \ell \concat (x \cons \Box)) & \text{otherwise}
  \end{cases}
\end{align*}
\end{definition}
...
The function: 
\[
  \den{} \index{$\den{\_}$}: \el (\tlist{grp} \times \bb{L}~\kw{int}) \ra \Z 
\]
...
& \text{with} \\
&\den_\bb{G} \index{$\den_\bb{G}$}: \el (\kw{grp} \times \bb{L}~\kw{int}) \ra \Z\\
&\den_\bb{G}{(\Cst(k), \_)} \re k \\
&\den_\bb{G}{(\Var{i}{k}, \ell)} \re \ell[i] * k \\
\end{align*}
%
\end{definition}
...
\begin{equation*}
\reify{\Box}{(x + 2 * y + x + 1)} \re^*
(
  \mathsf{Var}~0~1 \cons \mathsf{Var}~1~2 \cons \mathsf{Var}~0~1 \cons \cst{1} \cons \Box
  ,\
  \ell
)
\end{equation*}

The final context map $\ell = x \cons y \cons \Box$, so that $x$ has index $0$ and $y$ has index $1$.

\end{example}

\subsection{Outer normalisation procedure}

In this definition of merge sort algorithm, we reuse the split function defined in \cref{def:split} used for the \kw{contraction} rule.

\begin{definition}[Merge]
The function $\kw{merge}$ combines two lists of group elements by  folding constants and variables with the same index,
and produces a single list that is sorted by variable indices  assuming the inputs are themselves sorted.
\[
...
\begin{lemma}[Split correctness]\label{lem:la-split-correct}
...
\begin{lemma}[Merge correctness]\label{lem:la-merge-correct}
...
We can now combine the above invariants to show that mergesort preserves denotations.
...
\item[] (Case $n > 0$), with the definition of $\kw{mergesort}$ and $\kw{split}$, apply the induction hypothesis to both sublists with the bounds given by \cref{lem:size-split2}, and combine using \cref{lem:la-merge-correct} and \cref{lem:la-split-correct}.  
\end{itemize}
\end{proof}
\end{lemma}
...
\]
\begin{proof} Direct by induction on $x$ where cases are $\ZPos$, $\ZNeg$ and $\ZO$. \end{proof}
\end{theorem}
...
All definitions (reification $\Uparrow$, denotation $\Downarrow$, $\kw{merge}$, $\kw{mergesort}$, $\kw{remove0}$, and
...
\begin{align*}
& \bb{G}_\bb{R} \index{$\bb{G}_\bb{R}$}: \type \\
&|~\kw{var}: \N \ra \Q \ra \bb{G}_\bb{R} \\
&|~\kw{cst}: \Q \ra \bb{G}_\bb{R} \\
&\kw{rgrp} \index{$\kw{rgrp}$}: \set\\
...
The function $\Uparrow_\Q \index{$\Uparrow_\Q$} : \bb{L}~\kw{rat} \ra \Q \ra \el(\tlist{grp} \times \tlist{rat})$ is obtained from the integer version by replacing integer constants and coefficients with rational counterparts e.g. $\frac{0}{1}$ for $0$.
The denotation $\Downarrow_\Q \index{$\Downarrow_\Q$} : \el(\tlist{grp} \times \bb{L}~\kw{rat}) \ra \Q$ is unchanged apart from types: $\Cst(q)$ evaluates to $q \in \Q$ and $\Var{i}{q}$ evaluates to $\ell[i] *_\Q q$ for $\ell :\bb{L}~\kw{rat}$.
...
\begin{equation}
\label{eq:norm}
\tag{\textcolor{purple}{2}}
\kw{norm}\,(x : \bb{L}~\tt{grp}) 
  \;\coloneq\; 
  \eqnmarkbox[RoyalBlue]{rem0}{\kw{remove0}}
  \Bigl(\,
    \eqnmarkbox[lpurple]{msort}{\kw{mergesort}}\,x
  \Bigr)
\annotate[yshift=-0.5em]{below, left}{rem0}{cancel and removes neutral elements}
\annotate[yshift=-0.5em]{below, right}{msort}{sorts a list of $\tt{grp}$}
\end{equation}
