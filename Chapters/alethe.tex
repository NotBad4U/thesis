...
\begin{lstlisting}[language=SMT,caption={[Running SMT example and Alethe proof]Running example: an SMT problem and its Alethe proof found by cvc5.},label={lst:smtexampleinput-fol}]
(assume a0 (p a))
(assume a1 (= a b))
(assume a2 (not (p b)))
...
\paragraph{Subproofs}
The abstract notation in \cref{ssec:subproof-desc} marks subproofs with a vertical line.
...
\begin{table}
  ...
  \end{tabular}
  \caption[Linear arithmetic rules in Alethe]{Examples of linear arithmetic rules in Alethe.}
  \label{table:linear-arith-rules}
\end{table}

Proofs in linear arithmetic rely on a set of basic rules, summarized in \cref{table:linear-arith-rules}.
...
\begin{enumerate}
    ...
    This step puts the literal in a normalized form, by negating it when needed.
...
Formally, an Alethe proof $P$ is represented as a finite sequence $[C_1, \ldots, C_n]$ of steps and anchors.
