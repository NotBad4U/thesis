...
\section{Overview of Alethe Rules}\info{ac}{I will just repeat the rule here even if they are also describe in \cref{ch:reconstruction}}
...
Alethe provide different types of proof rules that we will describe with example of rule that  :
...
For example, the rule \tt{or\_simplify} below simplify \kw{or} expression:
...
The resolution rules also implicitly simplify repeated negations at the head of literals.
...
Existential quantifiers are handled by skolemization \question{ac}{should I say something also about skolemization?}.
...
Proofs for linear arithmetic use several straightforward rules listed in \cref{table:linear-arith-rules}, such as \tt{la\_totality} that asserts totality of the ordering $\leq$.
and the main rules \tt{la\_generic} and \tt{lia\_generic}, which describe an algorithm.
A step of the rule \tt{la\_generic} represents a tautological clause of linear inequalities.
That can be verified by demonstrating that the conjunction of the negated inequalities is unsatisfiable.
...
The proofs begins with \tt{assume} steps \tt{a0}, \tt{a1}, \tt{a2} that restate the assertions from the \emph{input problem}. % (\cref{lst:smtexampleproof}).
...
To elaborate on the \tt{la\_generic} rule, it can be verified by showing that the conjunction of the negated inequalities is unsatisfiable.
...
Let $\varphi_1,\dots, \varphi_n$ be rational numbers linear inequalities and $a_1, \dots, a_n$ rational numbers. Then a \tt{la\_generic} step has the general form
%
\[
\begin{matrix*}[c]
...
We present an abstract procedure to verify whether an Alethe proof
