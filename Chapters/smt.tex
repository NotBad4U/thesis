...
\]
\begin{itemize}
    \item  a finite set $\Theta^\cal{S}$ \index{$\Theta^\cal{S}$}  of sort symbols that contains the symbol \smtinline{Bool};
    \item The countably infinite set $\Theta^\cal{X}$ \index{$\Theta^\cal{X}$} contains the variables;
...
\begin{definition}[Satisfiability in a theory]
...
The standard method for propositional reasoning is \emph{conflict-driven clause learning} (CDCL) \index{CDCL} \cite{dpll}.
