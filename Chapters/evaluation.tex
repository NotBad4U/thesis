...
The benchmarks infrastructure repository\footnote{\url{https://ithub.com:NotBad4U/benchmarks-reconstruction.git}} and the operating system image has been published  to ensure reproducibility with a {grid'5000} ready for being reproduce by anyone with access to the platform,
...
The \cref{tab:benchmarks-pass-cvc5-carcara} summarizes the number of samples (column samples) for which cvc5 found a proof and that Carcara could elaborate.
...
Overall, our translator is able to produce Lambdapi scripts for $9898$ of them, that is, about $78\%$ of the elaborated samples.
...
We should increase the number of proofs reconstructed with little effort by supporting these rules.
...
If we admit the \tt{contraction} rule as an axiom instead of reconstructing them, then a lot of the proofs such as the ones in \tt{eq\_diamond}, previously timed out can be all checked successfully.
...
The benchmark \cref{tab:benchmarks-pass-times} belows report, for each benchmark samples, the distribution of checking times over all problems in that samples.
...
The \cref{fig:benchmarks-pass-times-boxplot} belows presents the values of \cref{tab:benchmarks-pass-times} with a box plot.
...
Similarly to the SMT-LIB benchmarks, the \cref{tab:benchmarks-tla} summarizes the number of samples that cvc5 found a proof for and that Carcara could elaborate.
...
For \tt{Cantor} contains proofs of Cantor’s theorem stating that the power set of any set has a strictly greater cardinality than the set itself,
...
This work was carried out in close collaboration with the Carcara developers: our benchmark suite revealed several bugs in the elaboration phase that we reported, leading to many fixes and thus strengthening the elaborater on which our reconstruction relies.
