...
This suite comprises six benchmark groups: \tt{LIA}, \tt{UFLIA}, \tt{QF\_UF}, \tt{UF}, \tt{QF\_LIA} , each corresponding to a distinct collection of proof obligations expressed in the SMT-LIB format that our encoding support.
In the Following, we will only present the results of the $26$ samples that has proofs that are checked by Lambdapi because we do not have enough space otherwise.
The complete results ($54$ samples), including failed and timed-out samples, are available in the \cref{app:complete-benchmarks-results}.
After we remove the samples that are not tag has \emph{unsat} in the header of the problem, we are left with a total of $31760$ samples.
The \cref{tab:benchmarks-pass-cvc5-carcara} summarizes the number of samples that cvc5 found a proof for and that Carcara could elaborate.
The column \tt{cvc5} and \tt{Carcara} use the format $S - F - T$ indicates respectively the number of success (S), failed (F) and timeout (T) samples.
...
In other words, Carcara successfully reconstructs about $97.4\%$ of the proofs produced by \texttt{cvc5} that reach the elaboration phase.
...
Over all benchmarks, Carcara successfully elaborates $13,028$ proof obligations, and our translator is able to produce Lambdapi scripts for $12,578$ of them, that is, about $96\%$ of the elaborated samples.
In the table, the column suffixed by \texttt{XL} corresponds to the proofs where we apply our \emph{parallel checking} technique to split the proof into several chunks and type check them independently described in \cref{ssec:parallel}.
The remaining $450$ failures have several causes. In the LIA benchmarks, some proofs cannot be translated because the underlying cvc5 proofs use the SMT-LIB operator \smtinline{to_real}, so that a problem stated over \tt{LIA} is ultimately solved in the \tt{LRA}; since our current encoding does not support \smtinline{to_real}, these samples are rejected.
...
Finally, the timeouts we observe, in particular for the parallel checking mode, are mainly caused by I/O bottlenecks in our current buffered writer: this configuration produces a large number of output files, and without further tuning the file writes become the dominating cost, even on our comparatively powerful server.
...
Compared to our previous work \cite{ColtellacciMD24} for first order logic, where we were not able to reconstruct any proof from the SMT-LIB benchmark families \texttt{20170428-Barrett} and \texttt{grasshopper},
our current reconstruction pipeline is now able to handle these families and successfully reconstruct all elaborated proofs from them.
...
In particular, we uncovered a performance bug in Lambdapi's \kw{rewrite} tactic: when applying our normalisation theorem (e.g. \cref{thm:contraction-correct}), the tactic becomes extremely slow, whereas directly normalising the same term with the Lambdapi command \lpinline{compute} succeeds, albeit still taking a noticeable amount of time.
If we admit the \tt{contraction} rule as an axiom instead of reconstructing them, then a lot of the proofs such as the ones in \tt{eq\_diamond}, previously timed out can be checked successfully.

Similarly, compared to our earlier work on linear integer arithmetic \cite{MeLIA}, our current reconstruction pipeline is able to handle new benchmark families such as \texttt{mathsat} and successfully reconstruct additional LIA proof obligations.
...
The benchmark \cref{tab:benchmarks-pass-times} report, for each benchmark samples, the distribution of checking times over all problems in that samples.
...
For every samples, we record the minimum (column \tt{min}), first quartile (column q1), median, third quartile (column q3), and maximum checking time (column \tt{max}), together with the arithmetic mean.
The \cref{fig:benchmarks-pass-times-boxplot} presents the values of \cref{tab:benchmarks-pass-times} with a box plot.
...
Most benchmark families have median checking times well below one thousand time units, while a few contain significantly harder instances with much larger maxima, highlighting the heterogeneous difficulty of the underlying proof obligations.
...
Overall, enabling our inner AC-based normalisation makes proof checking both faster.
...
Similarly to the SMT-LIB benchmarks, the table \cref{tab:benchmarks-tla} summarizes the number of samples that cvc5 found a proof for and that Carcara could elaborate.

The TLAPS benchmarks span a variety of typical algorithmic and distributed-system case studies.
The \tt{Allocator} \cite{allocator} models a simple resource allocator that enforces exclusive access to a finite pool of resources.
The \tt{EWD840} \cite{ewd840} encodes Dijkstra's token based termination detection algorithm on a ring of processes.
For \tt{Cantor} contains proofs of Cantor’s theorem and serves as a compact case study for higher order set theoretic reasoning.
...
Among these, cvc5 produces proofs for $339$ obligations, and Carcara successfully elaborates $300$ of them into Alethe.
...
Across all case studies, the weighted mean Lambdapi checking time per obligation lies between about $0.10$ s and $0.85$ s, and the median per benchmark ranges from roughly $0.10$s to $0.35$s, so most obligations are reconstructed quickly.
We are particularly encouraged that the vast majority of obligations are checked in well under one second, which is important because reconstruction is meant to be integrated into TLAPS and run while users are interactively developing proofs, so long checking times would directly harm the user experience.
...
such terms fall outside the intended first-order setting of Alethe and cannot be faithfully reconstructed.