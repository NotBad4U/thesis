...
\node[box, right=of carcara_elab] (carcara_recon) {Translation};
...
\paragraph{Software versions.} Our work is based on the following versions:
\begin{itemize} 
  \item cvc5 version $1.2.2-dev.277$ (commit 8aeaa1938)
  \item Carcara $1.1.0$ (commit 92fd8b4)
  \item Lambdapi version $3.0.0$ (commit 994691d)
\end{itemize}
...
\section[Parallel reconstruction and checking]{Parallel reconstruction and checking optimizations.}
\label{ssec:parallel}
...
The \texttt{translate / check} columns in \cref{tab:benchmarks-translate-small-check} report results for obligations whose Alethe proof is at most $10$ MB and are processed in the standard mode outputting a single file.
By contrast, the \texttt{translate XL / check XL} columns in \cref{tab:benchmarks-translate-large-check} report results for obligations whose Alethe proof exceeds $10$\,MB; for these large proofs we systematically apply our \emph{parallel checking} technique, splitting the proof into chunks and checking them independently.
...
\end{axis}
\end{tikzpicture}
\caption[Checking times across SMTLIB samples]{Lambdapi checking time variations between samples}
...
\begin{table}[t]
\centering
\caption[Lambdapi checking times]{Lambdapi checking times (in ms).}
{%
\scriptsize
\pgfplotstabletypeset[
  col sep=comma,
...
\begin{table}[t]
\centering
\caption[Checking times with AC]{Lambdapi checking times (ms) with \lpinline{associative commutative}.} 
{%
\scriptsize
\pgfplotstabletypeset[
  col sep=comma,
...
\section{Reconstruction of \texorpdfstring{\tlaplus}{TLA+} proofs}
...
\caption[Checking times across TLAPS samples]{Lambdapi checking time variations between TLAPS samples}
\end{figure}
